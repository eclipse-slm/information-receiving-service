...
class AasSource(BaseModel):
    ...
    def _request_shell_descriptors(self, cursor: str, limit: int) -> dict:
        args = {
            "limit": limit,
        ...
        return json.loads(
            self.shell_registry_client.get_all_asset_administration_shell_descriptors(**args).data
        )
    ...
    def _request_shells(self, cursor: str, limit: int) -> dict:
        ...
        response = self.shell_repository_client.get_all_asset_administration_shells(**args)
    ...
    def _request_submodels(self, cursor: str, limit: int) -> dict:
        ...
        response = self.submodel_repository_client.get_all_submodels(**args)
        return json.loads(response.data)


    def _request_submodel(self, identifier: str) -> dict:
    ...
    def _request_submodel_descriptors(self, cursor: str, limit: int) -> dict:
        args = {
            "limit": limit,
            "_preload_content": False
        }
        ...
        return json.loads(
            self.submodel_registry_client.get_all_submodel_descriptors(**args).data
        )


    def request_shell_descriptors(self) -> List[dict]:
        response = self._request_shell_descriptors(str(0), self._limit)
        return response['result']

    def request_shells(self) -> List[dict]:
